...
(function () {
    ...
    ecnuXjHome.controller('XsExamCtrl', ['$uibModal', 'ecnuExamDao', 'FileExport', 'Prompt', XsExamCtrl]);
    function XsExamCtrl($uibModal, ecnuExamDao, FileExport, Prompt) {
        ...
        vm.deleteItem = isDeleteItem;
        vm.deleteItems = isDeleteItems;
        ...
        function isDeleteItems() {
            var buttons = [deleteItems, function () {

            }];
            var proMessage = {'msg': "是否确认删除选择的考题"};
            var type = {'type': 'fa-exclamation-triangle'};
            Prompt.customPrompt(type, proMessage, buttons, buttonNames);
        }

        function isDeleteItem(id) {
            Global_id = id;
            var buttons = [deleteItem, function () {

            }];
            var proMessage = {'msg': "是否确认删除该考题"};
            var type = {'type': 'fa-exclamation-triangle'};
            Prompt.customPrompt(type, proMessage, buttons, buttonNames);
        }
        ...
        function deleteItem() {
            ecnuExamDao.deleteExam(Global_id, vm.currentPage - 1, vm.numPerPage,
                function (response) {
                    vm.totalItems = response.totalElements;
                    vm.items = response.content;
                }
