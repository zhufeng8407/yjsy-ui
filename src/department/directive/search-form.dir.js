...
(function () {
    ...
    function searchLayout(pathUtils) {
        return {
            ...
            templateUrl: pathUtils.qualifiedPath("/department/directive/search-form.view.html")
        }
    }
    ...
    function searchController(ecnuMetaDataDao, PageState, schoolFilter, departmentFilter, QUERY_PARAMS) {
        ...
        vm.showRegistrationInfo = vm.searchConfig.isFee || vm.searchConfig.isCheckin || vm.searchConfig.isOwnFee ||
                                  vm.searchConfig.isPoor || vm.searchConfig.isDelay || vm.searchConfig.isNew;
        vm.showTimeInfo = vm.searchConfig.term || vm.searchConfig.regYear;

        vm.searchCondition.isNew='所有';
        vm.searchCondition.isDifficulty='所有';
        //判断是新生注册页面还是老生注册页面
        if (("autoNew" in vm.searchConfig) && vm.searchConfig.autoNew)
            vm.searchCondition.isNew =  String(PageState.isPageForNewbie());
        loadMetaData();
        ...
        function loadMetaData() {
            ...
            if (vm.searchConfig.educationType) {
                ecnuMetaDataDao.getDegree(function (response) {
                    ...
                    vm.educationTypes.splice(0, 0, {
                        name: QUERY_PARAMS.VALUE_STRING_ALL,
                        code: QUERY_PARAMS.VALUE_STRING_ALL
                    });
                    vm.searchCondition.educationType = vm.educationTypes[0].code;
                });
            }
            ...
            if (vm.searchConfig.discipline) {
                ecnuMetaDataDao.getDisciplineCategory(function (response) {
                    ...
                    vm.searchCondition.discipline = QUERY_PARAMS.VALUE_STRING_ALL;
                    getDisciplineMajor(vm.searchCondition.discipline);
                });
            }
        ...
        function getDisciplineMajor(discipline) {
            if (vm.searchConfig.major) {
                ecnuMetaDataDao.getDisciplineMajor(discipline, function (response) {
                    ...
                    vm.searchCondition.majorCode = QUERY_PARAMS.VALUE_STRING_ALL;
                    getMinorsOfMajor(vm.searchCondition.majorCode);
                });
            }
        }
        ...
        function getMinorsOfMajor(majorCode) {
            if (vm.searchConfig.minor) {
                ecnuMetaDataDao.getMinorsOfMajor(majorCode, function (response) {
                    vm.minors = response;
                    vm.minors.splice(0, 0, ["所有", QUERY_PARAMS.VALUE_STRING_ALL]);
                    vm.searchCondition.minorCode = QUERY_PARAMS.VALUE_STRING_ALL;
                });
            }
        }
        ...
        function getDepartment(schoolCode) {
            ...
            function processDeparts(response) {
                ...
                vm.departments.splice(0, 0, ["所有", QUERY_PARAMS.VALUE_STRING_ALL]);
                ...
                if (vm.departments != undefined && !vm.departments.length == 0) {
                    vm.searchCondition.departmentCode = vm.departments[0][1];
                }
                if (vm.readyHook) {
                    vm.readyHook();
                    vm.readyHook = null;
