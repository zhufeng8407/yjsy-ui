...
(function () {
    ...
    function ZhglController($uibModal, ecnuAccountDao, PathUtils) {
        ...
        function showAddPanel(index) {
            if (vm.otherRoles[index].domainLevel == "校级" || vm.otherRoles[index].domainLevel == "学生") {
                ecnuAccountDao.addRole(vm.accountId, vm.otherRoles[index].roleID, [], function (response) {
                    vm.accountRoles = accountRoleHandler(response.accountRoles);
                    vm.otherRoles.splice(index, 1);
                })
            }
            else if (vm.otherRoles[index].domainLevel == "导师") {
                $uibModal.open({
                    animation: true,
                    size: 'lg',
                    templateUrl: 'addStudentForSupervisorPanel.html',
                    controller: 'AddStudentForSupervisorCtrl',
                    controllerAs: 'addStudentForSupervisorCtrl',
                    windowClass: 'teacherPanel',
                    backdrop: 'static',
                    resolve: {
                        sc: vm,
                        index: index,
        ...
        function modifyPrivilege(index) {
            if (vm.accountRoles[index].domainLevel != '校级' && vm.accountRoles[index].domainLevel != '导师' && vm.accountRoles[index].domainLevel != '学生')
                $uibModal.open({
                    animation: true,
                    size: 'lg',
                    templateUrl: 'addRolePanel.html',
            ...
            else if (vm.accountRoles[index].domainLevel == '导师') {
                $uibModal.open({
                    animation: true,
                    size: 'lg',
                    templateUrl: 'addStudentForSupervisorPanel.html',
    ...
    //type=0为新增角色，type=1位修改角色
    ecnuAdminHome.controller('AddStudentForSupervisorCtrl',
        [
            '$uibModalInstance',
            'ecnuAccountDao',
            'sc',
            'index',
            'type',
            AddStudentForSupervisorCtrl
        ]);
    function AddStudentForSupervisorCtrl($uibModalInstance, ecnuAccountDao, sc, index, type) {
        var vm = this;
        var id;
        var role = sc.otherRoles[index];

        vm.staffs = [];
        vm.staffsId = [];

        vm.cancel = cancel;
        vm.searchStaff = searchStaff;
        vm.reset = reset;
            ...
            if (type == 1) {
                ecnuAccountDao.getDomains(sc.accountId, sc.accountRoles[index].roleID, function (response) {
                    response.forEach(function (value) {
                        vm.staffs.push({'name': value.name});
                        vm.staffsId.push({'id': value.id});
                    })
                });
            }
        }
        ...
        function searchStaff() {
            if (vm.sno.trim() == "")
                ...
                ecnuAccountDao.findStaffByUsername(vm.sno.trim(), function (res) {
                    if (res != "") {
                        vm.name = res.name;
                        id = res.id;
                    }
                    else
                        alert("未查询到导师，请检查用户名是否正确！")
                })
        }
        ...
        function deleteStaff(index) {
            ...
            vm.staffsId.splice(index, 1);
        }

        function addStaff() {
            if (id != "") {
                if (!isContainId(vm.staffsId, {'id': id})) {
                    vm.staffs.push({'name': vm.name, 'grade': vm.grade, 'unit': vm.unit});
                    vm.staffsId.push({'id': id});
                } else
                    alert("该导师已存在！")
            }
            else {
        ...
        function save() {
            if (type == 0)
                ecnuAccountDao.addRole(sc.accountId, sc.otherRoles[index].roleID, vm.staffsId
                    , function (response) {
                        sc.accountRoles = accountRoleHandler(response.accountRoles);
                        sc.otherRoles.splice(index, 1);
                        $uibModalInstance.dismiss('cancel');
                ...
                ecnuAccountDao.updateRole(sc.accountId, sc.accountRoles[index].roleID, vm.staffsId
                    , function (response) {
                        sc.accountRoles = accountRoleHandler(response.accountRoles);
                        $uibModalInstance.dismiss('cancel');
                    });
        ...
        function isContainId(staffsId, Id) {
            var flag = false;
            staffsId.forEach(function (value, index, array) {
                if (value.id == Id.id)
                    flag = true;
            });
            return flag;
